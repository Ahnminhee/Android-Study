...
public class MainActivity extends AppCompatActivity {
    ...
    public void test() {
        String test = endPoint + "?serviceKey=" + key + "&busRouteId=30300001&";
        setUrlNParser(test);
    ...
    public boolean exminData() {
        if (busNum.equals("") || station.equals("")) {
            Toast.makeText(this, "값을 입력해주세여", Toast.LENGTH_SHORT).show();
            return true;
        }
        ...
        if (matcher_busNum.find() == false) {
            Toast.makeText(this, "버스 번호 다시 입력해주세요", Toast.LENGTH_SHORT).show();
            return true;
        }
        Matcher matcher_station = pattern_symbol.matcher(station);
        if (matcher_station.find() == false) {
            Toast.makeText(this, "정류장 번호 다시 입력해주세여", Toast.LENGTH_SHORT).show();
            ;
            return true;
        }
